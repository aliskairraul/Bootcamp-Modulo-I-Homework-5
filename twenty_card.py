...
def show_cards(cards: list, card_round: str) -> None:
    """Función que se encarga de dibujar las cartas del Juego

    Args:
        cards (list): Cartas a mostrar
        card_round (str): ronda
    """
...
def twenty_cards(number: int) -> tuple[bool, int]:
    """Funcion que se encarga de la logica del Juego Twenty_Cards.
       Es llamada desde el principal main.py y

    Args:
        number (int): es el numero de cartas que eligió el usuario sacar

    Returns:
        tuple[bool, int]: devuelve un Booleano que le indica si GANO o NO
        y un Entero con la puntuacion del jugador

    A tomar en Cuenta:
        .- lista con todas las posibles cartas entre 1 y 20
        .- Utilizo Strings en vez de valores númeicos para poder que todos sean
           de longitud 2 y queden igual de centrados en las cartas

    VARIABLES A TOMAR EN CUENTA:
    gano:bool                 --> Regresara a main.py el resultado de ganar o perder
                                  partida segun criterios del homework
    selected_cards:list = []   -> contendra la lista de cartas que selecciono el
                                  jugador (la n primeras cartas)
    sum_exposed_cards:int = 0  -> sumatoria de selected_cards
    missing_cards_up_to_50:int = 0 -> Cuantas cartas pudiera haber sacado extras sin llegar
                                      a 50. En caso de haber ganado
    next_card:int = 0           -> se usa para ir sacando las cartas de a una, para llegar
                                   a 50. Al final se queda con la carta que sobrepasa los 50
    next_cards:list = []        -> Lista de cartas extras que hubiese podido sacar sin
                                   llegar a 50
    puntuacion:int = 0          -> Puntuacion del jugador.. se calcula si gano..
                                   de lo contrario se queda 0
    """
    cards_list: list = [
        "01",
        "02",
    ...
    while True:
        # En este ciclo, el ultimo valor de next_card (que no entrara en el siguiente if)
        # es la carta que sobrepasa los 50
