...
def fill_containers_dicts(number: int, container: int) -> None:
    """Asigna los valores String a los diccionarios necesarios para la
       representacion grafica

    Args:
        number (int): representa la cantidad de valores del diccionario
                      a los que se les asignara 7*"░"
        container (int): indica a que container hace mencion
    """
...
def display_screen(gano: bool = False) -> int:
    """Funcion que se encarga de renderizar Toda la pantalla y lee la
       opcion del usuario

    Args:
        gano (bool, optional): Se usa para poder renderizar una vez haya ganado el usuario
                               y retornar inmediatamente sin esperar opcion del usuario
                               Defaults to False.

    Returns:
        int: retorna 0 si el usuario gano. O Retorna la opcion que escoge el usuario
    """

    option: int = 0
    menu_containers_game = """
                             TWO CONTAINERS GAME
    ...
    while True:
        ...
        # Si el usuario ya encontro la solucion (Ganó) despues de renderizar retorna a
        # la funcion que lleva la logica de la app
        if gano:
            return 0
...
def two_containers() -> tuple:
    """Funcion que se encarga de llevar la logica del Juego TWO_CONTAINERS,
       es llamada desde la main.py

    Returns:
        tuple: Retorna True y numero de intentos si el usuario Gano
               Retorna False y 0 si el usuario perdio
    """
    add: int = 0
    option: int = 0
    attempts: int = 0
